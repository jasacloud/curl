...
class Curl{
	...
	protected $headers = array(
		'Accept: text/xml,application/xml,application/json,application/xhtml+xml,text/html;q=0.9,text/plain;q=0.8,image/png,*/*;q=0.5',
     ...
     * @return string
     */
    public function getx($httpserver, $data = array())
    {
		$this->setHTTPURL($httpserver);
        $url = $this->baseUrl;
    ...
    public function post($url, $data = array(), $follow_303_with_post = false)
    {
	 ...
	 public function postx($httpserver, $data = array(), $follow_303_with_post = false)
    {
		$this->setHTTPURL($httpserver);
        $url = $this->baseUrl;
        $this->setURL($url);
    ...
    public function setDefaultUserAgent()
    {
        ...
        $user_agent = 'GSky-Client/' . self::VERSION . ' (+https://www.gemanusa.com/)';
	...
	public function setXForwardedFor($xForwardedForEnable=false){
		if($xForwardedForEnable && $_SERVER){
			$HTTP_X_FORWARDED_FOR = isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? [$_SERVER['HTTP_X_FORWARDED_FOR']] : [];
			$this->setHeader("X-Forwarded-For",implode(',',array_merge($HTTP_X_FORWARDED_FOR,[$_SERVER['REMOTE_ADDR']])));
		}
	}
	...
	public function setHTTPURL($httpserver=NULL){
		$this->baseUrl = $httpserver["protocol"]."://".$httpserver["host"].":".$httpserver["port"]."/".$httpserver["uri"];
	}
	...
	/*
		...
		@target = array(
			[
				'host'=>'http://10.250.191.108/api/system/clientparams',
				'method'=>'POST',
				'body'=>'<stringbody>',
				'header'=>[
			...
			'http://10.250.191.108/api/hiq/test'
		)
	*/
	public function asyncCurl($target=null){
		$this->mrHandler = new Handler();

		$this->mrHandler->setConnectionsLimit(1000);

		//$this->mrHandler->onRequestComplete('debugRequestComplete');
		//$this->mrHandler->onRequestComplete('saveCompleteRequestToFile');
		$this->mrHandler->onRequestComplete(function(Request $request, Handler $handler){
			session_start();
			if(!isset($_SESSION['response'])){
				...
				$_SESSION['response'] = array_merge($_SESSION['response'],array([
					'http_code'=>$request->getCode(),
					'header'=>$request->getResponseHeaders(),
					'cookies'=>$request->getResponseCookies(),
					'body'=>$request->getContent(),
		...
		//$this->session = new \MultiRequest\Session($this->mrHandler, '/tmp');
		//$this->session->start();
		session_start();
		...
		foreach($target as $value) {
			...
			if(is_array($value)){
				$this->request = new Request($value['host']);
				$this->request->addCurlOptions($this->curlOptions);
				if(isset($value['method']) && $value['method'] == "POST"){
					$this->request->setCurlOption(CURLOPT_POST,true);
					$this->request->setCurlOption(CURLOPT_POSTFIELDS, $this->buildPostData($value['body']));
				}
				if(isset($value['header']) && is_array($value['header'])){
					$this->request->addHeaders($value['header']);
				}
				//$this->session->request($this->request);
				$this->mrHandler->pushRequestToQueue($this->request);
			}
			else{
			
				$this->request = new Request($value);
				$this->request->addCurlOptions($this->curlOptions);
		...
		new Logger($_SERVER['DOCUMENT_ROOT'].'/log/MultiRequest.log', $_SERVER['PHP_SELF'].':'.__LINE__.'  '.'[Curl::setHandler()] '.'Total time: ' . (time() - $startTime));
		return [
			"returnval"=>true,
